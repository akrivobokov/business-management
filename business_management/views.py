...
from orders.models import Banner, Invoice, InvoiceSequence, PixelOrder
...
def landing(request):
    banners = list(
        Banner.objects.filter(is_active=True, is_compliant=True).values(
            'id', 'title', 'image_url', 'target_url', 'cells'
        )
    )
    return render(
        request,
...
def create_pixel_order(request):
    ...
    cell_count = _parse_positive_int(payload.get('cell_count'), 0)
    if cell_count <= 0:
        return JsonResponse({'error': 'Укажите корректное количество ячеек'}, status=400)

    consent = bool(payload.get('personal_data_consent'))
    if not consent:
